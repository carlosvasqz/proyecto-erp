...
<?php 
    ...
    if($rowVentaTmpExiste['Existe_Venta']>0){
        ...
        $queryModificarVentaTmp=mysqli_query($db,"UPDATE ventas_tmp SET Id_Cliente = '$codigo_cliente', Id_Usuario = '$codigo_usuario', fecha = '$fecha', Sub_Total = 0, Descuento = 0, ISV = 0, Total = 0 WHERE Id_Venta_Tmp=$codigo_venta_tmp;")or die(mysqli_error());
        // echo "Ha sido actualizada\n";
    } else { 
        // echo "La factura no existe\n";
        // Insercion de una nueva factura en la tabla de facturas temporales
        $queryInsertarVentaTmp=mysqli_query($db,"INSERT INTO ventas_tmp(Id_Venta_Tmp, Id_Cliente, Id_Usuario, Fecha, Sub_Total, Descuento, Impuesto, Total) VALUES('$codigo_venta_tmp', '$codigo_cliente', '$codigo_usuario', '$fecha', 0, 0, 0, 0);")or die(mysqli_error());
        // echo "Ha sido creada\n";
    }                                                                                                   
