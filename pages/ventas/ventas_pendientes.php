...
<head>
  ...
  <title>MaterialAdminLTE 2 | Ventas Pendientes</title>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li class="active">Ventas Pendientes</li>
      </ol>
    </section>
          ...
          <div class="box">
            <div class="box-header">
             ...
             <div class="pull-right box-tools">
                <button type="button" class="btn btn-info" id="btnRegistrarNuevo">
                  <i class="fa fa-plus"></i> <b>Registrar Nueva Venta</b></button>
              </div>
              <!-- /. tools -->
            </div>
            <!-- /.box-header -->
            <div class="box-body">
              <table id="lista" class="table table-bordered table-striped table-hover">
                ...
                <tbody>
                  <?php
                    $queryVentasTmp=mysqli_query($db, "SELECT * FROM ventas_tmp") or die(mysqli_error());
                    while ($rowVentaTmp=mysqli_fetch_array($queryVentasTmp)) {
                      echo '
                        <tr>
                            <td>'.$rowVentaTmp['Id_Venta_Tmp'].'</td>
                            <td>'.$rowVentaTmp['Id_Cliente'].'</td>
                            ...
                            <td>
                              <form action="registro_ventas.php" method="POST">
                                <input type="hidden" name="codigo_venta_tmp" value="'.$rowVentaTmp['Id_Venta_Tmp'].'"/>
                                <button type="submit" class="btn btn-primary btn-sm" data-toggle="tooltip" title="Reutilizar"><i class="fa fa-external-link-square"></i></button>
                              
                                <button type="button" id="'.$rowVentaTmp['Id_Venta_Tmp'].'" class="btn btn-danger btn-sm sweetalert" data-toggle="tooltip" title="Descartar"><i class="fa fa-trash"></i></button>
                              </form>
                            </td>
                        </tr>
                      ';
...
<script>
  ...
  $(document).ready(function () {
    ...
    $('.sweetalert').click(function(){
      ...
      swal({
          title: "¿Esta seguro?",
          text: "Esta accion Descartará la Venta Temporal seleccionada",
          type: "warning",
          ...
          $.ajax({
            //Direccion destino
            url: "ventas_pendientes_descartar.php",
            // Variable con los datos necesarios
            data: "Id_Venta_Tmp=" + idVenta,
            type: "POST",			
            dataType: "html",
            //cache: false,
            //success
              ...
              setTimeout(function () {
                if (data) {
                  swal({
                    ...
                    if (isConfirm) {
                      window.setTimeout('location.href="ventas_pendientes.php"', 3);
                    }
                  });
                }
                if (!data) {
